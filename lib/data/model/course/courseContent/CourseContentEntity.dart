import 'package:edwisely/data/model/course/courseContent/academic_materials.dart';
...
class CourseContentEntity {
  ...
  int status;
  String message;
  List<Academic_materials> academic_materials;
  List<Learning_content> learning_content;

	CourseContentEntity.fromJsonMap(Map<String, dynamic> map): 
		status = map["status"],
		message = map["message"],
		academic_materials = List<Academic_materials>.from(map["academic_materials"].map((it) => Academic_materials.fromJsonMap(it))),
		learning_content = List<Learning_content>.from(map["learning_content"].map((it) => Learning_content.fromJsonMap(it)));
