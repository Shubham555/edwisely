...
class CreateVCScreen extends StatelessWidget {
//   @override
...
// class _CreateVCScreenState extends State<CreateVCScreen> {
  ...
  Future<DateTime> _pickEndDate(BuildContext context) async {
    return showDatePicker(context: context, initialDate: DateTime.now(), firstDate: _vcStart, lastDate: _vcStart).catchError(() {
      _vcEnd = null;
    });
  }
  ...
  Future<TimeOfDay> _pickEndTime(BuildContext context) async {
    return showTimePicker(
      ...
      if (value.hour - _vcStartTime.hour > 3) {
        Get.defaultDialog(title: 'Select time less than 3 hours', onConfirm: () => Get.back(), middleText: '');
      }
    });
  }
                                                ...
                                                children: [
                                                  Row(
                                                    ...
                                                    children: [
                                                      ...
                                                      InkWell(
                                                        onTap: () async {
                                                          _vcStart = await _pickStartDate(context);
                                                          _vcStartTime = await _pickStartTime(context);
                                                          setState(() {});
                                                        },
                                                        child: Text(
                                                        ...
                                                        onTap: () async {
                                                          if (_vcStart == null) {
                                                            _scaffoldKey.currentState
                                                                .showSnackBar(SnackBar(content: Text('Select Start Date First !')));
                                                          } else {
                                                            _vcEnd = await _pickEndDate(context);
                                                            _vcEndTime = await _pickEndTime(context);
                                                            setState(() {});
                                                          }
                                                        },
                                                        child: Text(
